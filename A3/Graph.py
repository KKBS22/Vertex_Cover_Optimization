...
class Graph:
    ...
    def vertexInfo_a2(self):

        pass

    def graphInfo_a2(self):
        pass
    ...
    def displayGraph_a2(self,modList):
        finalList = []
        print('E {', end="")
                    ...
                    if type(value[0]) is list:
                        ...
                        for k in range(len(subLista)):
                            for a in range(len(modList)):
                                if (keys == modList[a][0]):
                                    keys = modList[a][1]
                                if (subLista[k][0] == modList[a][0]):
                                    subLista[k][0] = modList[a][1]                
                            print('<{0},{1}>,'.format(keys,subLista[k][0]),end="",file=sys.stdout,flush=True)                
                if len(value)==2:
                    if type(value[0]) is list:
                        if len(value[0]) >=2:
                            aKey = value[0][0]
                    ...
                    if type(value[0]) is list:
                        ...
                        final = g.union(f)
                        lfinal = [list(i) for i in final]
                        for k in range(len(list(final))):
                            for a in range(len(modList)):
                                if (keys == modList[a][0]):
                                    keys = modList[a][1]
                                if (lfinal[k][0] == modList[a][0]):
                                    lfinal[k][0] = modList[a][1]                    
                            print('<{0},{1}>,'.format(keys,lfinal[k][0]),end="",file=sys.stdout,flush=True)
            if lenTest ==v:
                if len(value)>=2:
                    if type(value[0]) is list:
                        1+1
                                            ...
                                            if vI == totalStreets[ts].streetName:
                                                ...
                                                if statusStreet == True:
                                                    if self.betweenStreet(pointOnStreet,value2.point):
                                                        ...
                                                        presentStatus = self.checkPresence(dictV)
                                                        presentStatusVV = self.checkPresenceVV(dictV)

                                                        if (presentStatus == False) and (presentStatusVV == False):
    ...
    def checkPresence(self,dictToInsert):
        present = False
        for kPV, vPV in self.graphList.items():
            if len(self.graphList[kPV])>=2:
                aK = vPV[0]
                aV = vPV[1]
                sLA =[[vPV[0],vPV[1]]]
                if len(self.graphList[kPV])>2:
                    nL = vPV[2:len(self.graphList[kPV])]
                    for j in nL:
                        sLA.append(j)
                for a in range(len(sLA)):
                    for kD, vD in dictToInsert.items():
                        if ((vD[0] == kPV)and(kD == sLA[a][0])):
                            present = True
        return present

    def checkPresenceVV(self,dictToInsert):
        present = False
        for kPV, vPV in self.graphListVV.items():
            if len(self.graphListVV[kPV])>=2:
                aK = vPV[0]
                aV = vPV[1]
                sLA =[[vPV[0],vPV[1]]]
                if len(self.graphListVV[kPV])>2:
                    nL = vPV[2:len(self.graphListVV[kPV])]
                    for j in nL:
                        sLA.append(j)
                for a in range(len(sLA)):
                    for kD, vD in dictToInsert.items():
    ...
    def findcollinear(self,streetA, streetB, streetC): 
        lengthAB = self.findLength(streetA, streetB)
        lengthBC = self.findLength(streetB, streetC)
        lengthAC = self.findLength(streetA, streetC)
        p = (lengthAB + lengthBC + lengthAC)/2
        area = abs((p*(p-lengthAB)*(p-lengthAC)*(p-lengthBC)))** 0.5
        #area = streetA[0] * (streetB[1] - streetC[1]) + streetB[0] * (streetC[1] - streetA[1]) + streetC[0] * (streetA[1] - streetB[1]) 
        area2 = abs(round(area,4))
                                ...
                                if len(self.graphList[keyj])>=2:
                                    ...
                                    for a in range(len(subLista)):
                                        if dictVal[keyi] not in subLista:
                                            ...
                                            if state == True:
                                                if len(self.graphList[keyj])>2:
                                                    self.graphList[keyj].append(dictVal[keyi])
                                                else:
                                                    self.graphList[keyj] = dictVal[keyi]
                                            elif colTest == 0:
